  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
      ...
      <div className="bg-[rgba(0,0,0,0.8)] rounded-lg p-4 border border-[rgba(0,255,136,0.3)]">
        <div className="text-sm text-[var(--primary)] font-bold mb-3">Countermeasures:</div>
            ...
            return (
              <div key={key} className="relative">
                <button
                  ...
                  style={{
                    backgroundColor: isSelected ? countermeasure.color : undefined
                  }}
                >
                  <div className="flex justify-between items-center">
                    <span>{key === 'kinetic' ? 'Kinetic' : key === 'electronic' ? 'EW' : 'Laser'}</span>
                    <div className="flex items-center gap-1 text-xs">
                      <span>R:{countermeasure.range}</span>
                      {countermeasure.ammo !== undefined && (
                        <span className={hasAmmo ? 'text-green-400' : 'text-red-400'}>
      ...
      {/* Game Controls */}
      <div className="bg-[rgba(0,0,0,0.8)] rounded-lg p-4 border border-[rgba(0,255,136,0.3)]">
        <div className="text-sm text-[var(--primary)] font-bold mb-3">Game Controls:</div>
        ...
        <div className="mt-2 text-xs text-gray-400">
          Debug: Active: {gameState.activeThreats} / Max: {gameState.maxThreats} | Can Generate: {canGenerateSwarm ? 'Yes' : 'No'}
        </div>
      </div>
      ...
      <div className="bg-[rgba(0,0,0,0.8)] rounded-lg p-4 border border-[rgba(0,255,136,0.3)]">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-green-400 font-bold">{gameState.neutralizedCount}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between border-t border-[rgba(0,255,136,0.2)] pt-2">
            ...
            <span className="text-blue-400 font-bold">{gameState.gameLevel}</span>
          </div>
        </div>
      </div>
    </div>
