...
export const ThreatSimulator: React.FC<ThreatSimulatorProps> = ({ className = "" }) => {
  ...
  const neutralizeThreat = (threat: HTMLElement, countermeasureType?: string) => {
    if (threat.classList.contains('neutralized')) return;
    
    const usedCountermeasure = countermeasureType || gameState.selectedCountermeasure;
    const countermeasure = countermeasures[usedCountermeasure];
    const currentThreatType = threatTypes[threat.dataset.type as keyof typeof threatTypes];
    ...
    const effectiveness = countermeasure.effectiveness[threat.dataset.type || 'drone'] || 0.5;
  ...
  const startAutomatedThreat = (threat: HTMLElement) => {
    ...
    const threatTypeData = threatTypes[threat.dataset.type as keyof typeof threatTypes];
    ...
    const moveTowardShield = () => {
      if (threat.classList.contains('neutralized') || !gameState.gameRunning) return;
      ...
      for (const [cmType, cm] of Object.entries(countermeasures)) {
        if (cm.autoTrigger && canUseCountermeasure(cmType) && distance <= cm.range) {
  ...
  const { addDragFunctionality, handleMouseMove, handleMouseUp } = useDragAndDrop(
    simulatorRef,
    neutralizeThreat,
  ...
  }, [gameState.gameRunning, gameState.threatSpawnRate, handleMouseMove, handleMouseUp, spawnThreat]);

  // Handle fullscreen toggle - only for the game container
  useEffect(() => {
    console.log('Fullscreen effect triggered, isFullscreen:', gameState.isFullscreen);
