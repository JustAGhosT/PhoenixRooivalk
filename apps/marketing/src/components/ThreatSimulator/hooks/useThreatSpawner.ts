...
export const useThreatSpawner = (
  ...
  const spawnThreat = useCallback(() => {
    if (!gameRunning || activeThreats >= maxThreats || !simulatorRef.current)
      return;
    ...
    threat.className =
      "absolute w-8 h-8 rounded-full flex items-center justify-center cursor-grab hover:cursor-grabbing hover:scale-110 transition-all shadow-lg threat-moving";
  ...
  const generateSwarm = useCallback(() => {
    if (!gameRunning || !simulatorRef.current) {
      console.log("Swarm generation blocked:", {
        gameRunning,
        hasRef: !!simulatorRef.current,
      });
      return;
    }
    ...
    if (availableSlots <= 0) {
      console.log("No available slots for swarm:", {
        activeThreats,
        maxThreats,
      });
      return;
    ...
    for (let i = 0; i < swarmSize; i++) {
      setTimeout(() => {
        ...
        threat.id = `threat-${threatCounter.current}`;
        threat.className =
          "absolute w-8 h-8 rounded-full flex items-center justify-center cursor-grab hover:cursor-grabbing hover:scale-110 transition-all shadow-lg threat-moving swarm-threat";
        threat.style.backgroundColor = threatType.color;
        threat.style.zIndex = "10";
        threat.dataset.type = randomType;
        threat.dataset.health = threatType.health.toString();
