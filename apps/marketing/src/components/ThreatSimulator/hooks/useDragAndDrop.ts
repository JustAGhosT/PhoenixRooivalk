...
export const useDragAndDrop = (
  ...
  const addDragFunctionality = useCallback(
    (threat: HTMLElement) => {
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      const distance = Math.sqrt(
        (threatCenterX - centerX) ** 2 + (threatCenterY - centerY) ** 2,
      );

      const countermeasure = countermeasures[selectedCountermeasure];
      if (
        countermeasure &&
        distance <= countermeasure.range &&
        canUseCountermeasure(selectedCountermeasure)
      ) {
