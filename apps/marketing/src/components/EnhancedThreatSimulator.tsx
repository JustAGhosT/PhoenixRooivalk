...
> = ({ className = "" }) => {
  ...
  const [friendlyDeployments, setFriendlyDeployments] = useState<
    FriendlyDeployment[]
  >([]);
  ...
  const handleEffectorSelection = useCallback(
    (effectorId: string) => {
      const effector = effectorDatabase.effectors.find(
        (e) => e.id === effectorId,
      );
      if (!effector) return;
  ...
  const handleDroneSelection = useCallback(
    (selected: string[], _usedEnergy: number) => {
  ...
  const isPlanningComplete = useCallback(() => {
    ...
    const hasLegalAcknowledgment = selectedEffectors.every((effectorId) => {
      ...
      return effector.legal_flags.every((flag) =>
    ...
    <div className={`enhanced-threat-simulator ${className}`}>
      <div className="simulator-header">
        <h1 className="simulator-title">
          SENTINEL CORE (EDN-1) - THREAT SIMULATOR
        </h1>
            ...
            <div className="effector-grid">
              {getFilteredEffectors().map((effector) => {
                ...
                const needsLegalAck =
                  ...
                  effector.legal_flags.some(
                    (flag) => !acknowledgedLegal.includes(flag),
                  );
                ...
                return (
                  ...
                  >
                    ...
                    <div className="effector-controls">
                      <CompactROEIndicator
                        riskLevel={effector.roe as "low" | "medium" | "high"}
                      />
                      ...
                      {effector.legal_flags && (
                        <LegalBadge
                          ...
                          onAcknowledge={() =>
                            handleLegalAcknowledgment(effector.id)
                          }
                        />
                      )}
              ...
              return (
                <div key={effectorId} className="active-system-card">
                  ...
                  <div className="system-controls">
                    ...
                    <button
                      className="fire-btn"
                      disabled={currentEnergy < (effector.energy || 0)}
                    >
                      FIRE
                    </button>
                  </div>
