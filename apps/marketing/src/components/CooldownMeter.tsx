...
export const CooldownMeter: React.FC<CooldownMeterProps> = ({
  ...
  const progress = cooldown > 0 ? Math.max(0, Math.min(1, (cooldown - remaining) / cooldown)) : 1;
      ...
      <div className="cooldown-meter-container">
        ...
        <div className="cooldown-meter-content">
          {isReady ? (
            ...
            <span className="cooldown-meter-time" aria-label={`${remainingSeconds} seconds remaining`}>
              {remainingSeconds}
            </span>
          )}
        </div>
