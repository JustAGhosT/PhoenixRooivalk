...
export function circleCircleCollision(
  ...
  if (dist < combinedRadius) {
    ...
    const collisionPoint = {
      x: circle1.x + (circle2.x - circle1.x) * (circle1.radius / combinedRadius),
      y: circle1.y + (circle2.y - circle1.y) * (circle1.radius / combinedRadius)
    };
...
export function rectangleRectangleCollision(
  ...
  if (overlapX > 0 && overlapY > 0) {
    const collisionPoint = {
      x: (Math.max(rect1.x, rect2.x) + Math.min(rect1.x + rect1.width, rect2.x + rect2.width)) / 2,
      y: (Math.max(rect1.y, rect2.y) + Math.min(rect1.y + rect1.height, rect2.y + rect2.height)) / 2
    };
...
export class CollisionSystem {
   ...
   */
  updateObject(id: string, x: number, y: number, velocity?: { x: number; y: number }): void {
   ...
   */
  checkCollisions(): Array<{ obj1: PhysicsObject; obj2: PhysicsObject; result: ImpactResult }> {
    const collisions: Array<{ obj1: PhysicsObject; obj2: PhysicsObject; result: ImpactResult }> = [];
   ...
   */
  private checkObjectCollision(obj1: PhysicsObject, obj2: PhysicsObject): ImpactResult {
