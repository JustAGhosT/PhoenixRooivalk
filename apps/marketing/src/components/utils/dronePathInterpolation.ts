...
export class DronePathInterpolator {
   ...
   */
  setTarget(targetX: number, targetY: number, currentX: number, currentY: number): void {
   ...
   */
  updatePosition(currentX: number, currentY: number): { x: number; y: number; velocity: { x: number; y: number } } {
    if (!this.currentPath) {
    ...
    const desiredSpeed = Math.sqrt(desiredVelocity.x ** 2 + desiredVelocity.y ** 2);
    if (desiredSpeed > this.config.maxSpeed) {
      desiredVelocity.x = (desiredVelocity.x / desiredSpeed) * this.config.maxSpeed;
      desiredVelocity.y = (desiredVelocity.y / desiredSpeed) * this.config.maxSpeed;
    }

    // Smooth velocity changes
    this.currentVelocity.x += (desiredVelocity.x - this.currentVelocity.x) * this.config.smoothing;
    this.currentVelocity.y += (desiredVelocity.y - this.currentVelocity.y) * this.config.smoothing;
    ...
    const distanceToTarget = distance(newPosition.x, newPosition.y, targetX, targetY);
    ...
    return { x: newPosition.x, y: newPosition.y, velocity: this.currentVelocity };
  }
