...
pub fn SynergySystem(
    ...
    let active_synergies = move || {
        let active = active_weapons.get();
        synergies
            .iter()
            .filter(|synergy| synergy.weapons.iter().all(|w| active.contains(w)))
            .cloned()
            .collect::<Vec<_>>()
    };
        ...
        <Show when=move || show.get() fallback=|| view! { <div></div> }>
            <div class="synergy-indicator">
                <div class="synergy-header">
                    ...
                    <span class="synergy-count">{move || active_synergies().len()}</span>
                </div>

                <div class="synergy-list">
                    <For
                        each=move || active_synergies()
                ...
                <Show
                    when=move || active_synergies().is_empty()
